...
#if defined(NDEBUG)
#error "Specialcoin cannot be compiled without assertions."
#endif
        ...
        if(tx.IsZerocoinSpend()){
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zSPCC spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight > 0 && nHeight <= 21600) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Specialcoin
...
{
    RenameThread("specialcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSPCCMinted()
{
...
void RecalculateZSPCCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSPCC supply
...
bool RecalculateSPCCSupply(int nHeightStart)
{
...
bool UpdateZSPCCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (Params().Zerocoin_Block_RecalculateAccumulators() >= 0 && pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSPCCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSPCC money supply in the block index
    if (!UpdateZSPCCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zSPCC supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SPCC to zSPCC
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Specialcoin: We use certain sporks during IBD, so check to see if they are
