...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SPCC);
    ...
    if (!settings.contains("nAnonymizeSpecialcoinAmount"))
        settings.setValue("nAnonymizeSpecialcoinAmount", 1000);

    nAnonymizeSpecialcoinAmount = settings.value("nAnonymizeSpecialcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSpecialcoinAmount"))
        SoftSetArg("-anonymizespecialcoinamount", settings.value("nAnonymizeSpecialcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in specialcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSpecialcoinAmount:
            return QVariant(nAnonymizeSpecialcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSpecialcoinAmount:
            nAnonymizeSpecialcoinAmount = value.toInt();
            settings.setValue("nAnonymizeSpecialcoinAmount", nAnonymizeSpecialcoinAmount);
            emit anonymizeSpecialcoinAmountChanged(nAnonymizeSpecialcoinAmount);
            break;
        case CoinControlFeatures:
